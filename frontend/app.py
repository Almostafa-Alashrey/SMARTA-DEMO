...
# --- Load YOLO Model for Frontend Live View ---
@st.cache_resource
def load_yolo_model():
    return YOLO("yolov8n.pt")

live_model = load_yolo_model()
...
def check_system_notifications():
    # 1. Check Inventory Expiry (3, 2, 1 days remaining)
    try:
        inv_res = requests.get(f"{BACKEND_URL}/api/v1/inventory", timeout=2).json()
        if inv_res.get("success") and inv_res.get("inventory"):
            for item in inv_res["inventory"]:
                ...
                if days_left == 3:
                    st.toast(f"⚠️ Warning: {item_name} on {shelf} expires in 3 days!", icon="⏳")
                elif days_left == 2:
                    st.toast(f"🚨 Urgent: {item_name} on {shelf} expires in 2 days!", icon="⚠️")
                elif days_left <= 1:
                    st.toast(f"❌ Critical: {item_name} on {shelf} expires in 1 day or less!", icon="🔥")
    except Exception:
        pass

    # 2. Check Telemetry Parameter Anomalies (Temp, Humidity, Gas)
    try:
        tele_res = requests.get(f"{BACKEND_URL}/api/v1/telemetry", timeout=2).json()
        if tele_res.get("success") and tele_res.get("telemetry"):
            latest = tele_res["telemetry"][0]
            ...
            if is_anomaly == 1:
                st.toast(f"🚨 Microclimate Anomaly Detected at {loc}!", icon="⚡")
                if temp > 30.0:
                    st.toast(f"🔥 High Temperature Alert at {loc}: {temp}°C", icon="🌡️")
                if hum > 75.0:
                    st.toast(f"💧 High Humidity Alert at {loc}: {hum}%", icon="💧")
                if gas > 1000.0:
                    st.toast(f"☁️ Hazardous Gas Spike at {loc}: {gas} PPM", icon="☁️")
    except Exception:
        pass

# Trigger notification check on load/rerun
...
# --- WebRTC Video Processor for Real-Time Bounding Boxes ---
class LiveVeggieProcessor(VideoProcessorBase):
    def recv(self, frame: av.VideoFrame) -> av.VideoFrame:
        img = frame.to_ndarray(format="bgr24")
        results = live_model(img, conf=0.4)
...
with tab1:
    ...
    if not df.empty and 'location' in df.columns:
        ...
        if not display_df.empty:
            ...
            k2.markdown(f'<div class="glass-card"><div class="glass-label">Temperature</div><div class="glass-value">{latest["temperature"]} °C</div></div>', unsafe_allow_html=True)
            k3.markdown(f'<div class="glass-card"><div class="glass-label">Humidity</div><div class="glass-value">{latest["humidity"]} %</div></div>', unsafe_allow_html=True)
            k4.markdown(f'<div class="glass-card"><div class="glass-label">Methane / Gas</div><div class="glass-value">{latest["gas_level"]} ppm</div></div>', unsafe_allow_html=True)
            ...
            with ch3:
                ...
                st.plotly_chart(plot_spline_chart(display_df, 'gas_level', '#20c997', 'Gas'), use_container_width=True)
    else:
        st.info("Waiting for data from IoT Simulator... (Ensure Backend & Simulator are running)")
...
with tab2:
    ...
    input_mode = st.radio("Select Image Input Source:", ["📹 Real-Time Live Scanner", "📷 Snap Photo", "📁 Upload Image File"], horizontal=True)
    selected_shelf = st.selectbox("Assign Shelf Location", ["Shelf A1", "Shelf B2", "Shelf C3"])

    if input_mode == "📹 Real-Time Live Scanner":
        st.write("Click **Start** below to open your camera. Live bounding boxes and labels will appear instantly as you move items.")
        webrtc_streamer(
            key="live-scanner",
            video_processor_factory=LiveVeggieProcessor,
            media_stream_constraints={"video": True, "audio": False},
            rtc_configuration={"iceServers": [{"urls": ["stun:stun.l.google.com:19302"]}]}
        )
        st.info("💡 **Tip:** This provides direct, unrecorded real-time inference loop using your browser camera.")
        ...
        if uploaded_file and st.button("🔍 Run AI Diagnostics"):
            with st.spinner("Processing image via AI model..."):
                try:
                    ...
                    if res.get("success"):
                        st.success(f"✅ Item successfully assigned to shelf!")
